...
public class DiscussPostController implements CommunityConstant {
    ...
    public String getDiscussPost(@PathVariable("discussPostId") int discussPostId, Model model, Page page) {
        ...
        List<Comment> commentList = commentService.findCommentsByEntity(
                ENTITY_TYPE_POST, post.getId(), page.getCurrent(), page.getLimit()).getRecords();
        // 评论VO列表
        ...
        if (commentList != null) {
            for (Comment comment : commentList) {
                ...
                List<Comment> replyList = commentService.findCommentsByEntity(
                        ENTITY_TYPE_COMMENT, comment.getId(), 1, Integer.MAX_VALUE).getRecords();
                // 回复VO列表
    ...
    public String setDelete(int id) {
        ...
        // 触发删帖事件
        eventProducer.fireDiscussPostEvent(TOPIC_DELETE, hostHolder.getUser().getId(), id);
        return CommunityUtil.getJSONString(0);
    }
